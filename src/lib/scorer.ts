...
export function scoreToken(token: RawToken, config: BotConfig): ScoredToken {
  ...
  // --- Age (ultra-fresh rewarded hardest; most runners surface in first hour) ---
  if (ageMin < 30) {
    score += 20
    signals.push({ type: 'fresh_pair', strength: 'extreme', description: 'Pair <30m old', points: 20 })
  } else if (ageMin < 60) {
    score += 14
    signals.push({ type: 'fresh_pair', strength: 'strong', description: 'Pair <1h old', points: 14 })
  } else if (ageMin < 120) {
    score += 8
    signals.push({ type: 'fresh_pair', strength: 'moderate', description: 'Pair <2h old', points: 8 })
  } else if (ageMin < 240) {
    score += 3
    signals.push({ type: 'fresh_pair', strength: 'weak', description: 'Pair <4h old', points: 3 })
  } else {
    warnings.push(`Pair age is ${ageHours.toFixed(1)}h; older than ideal runner window`)
  }

  // --- Liquidity (deeper pools = safer entry + more gas for the runner) ---
  if (token.liquidity >= 200_000) {
    score += 18
    signals.push({ type: 'liquidity', strength: 'strong', description: `$${(token.liquidity / 1000).toFixed(0)}k liquidity`, points: 18 })
  } else if (token.liquidity >= 80_000) {
    score += 10
    signals.push({ type: 'liquidity', strength: 'strong', description: 'Liquidity above baseline', points: 10 })
  } else if (token.liquidity >= 10_000) {
    score += 5
    signals.push({ type: 'liquidity', strength: 'weak', description: `Early liquidity $${(token.liquidity / 1000).toFixed(1)}k`, points: 5 })
  } else {
    warnings.push(`Liquidity $${token.liquidity.toFixed(0)} below threshold`)
  }

  // --- Volume velocity (turnover relative to pool size — the #1 runner indicator) ---
  if (token.volume24h > 0 && token.liquidity > 0 && token.volume24h / token.liquidity >= 3.0) {
    score += 20
    signals.push({ type: 'volume_velocity', strength: 'extreme', description: `Vol/Liq ${(token.volume24h / token.liquidity).toFixed(1)}x`, points: 20 })
  } else if (token.volume24h > 0 && token.liquidity > 0 && token.volume24h / token.liquidity >= 1.8) {
    score += 10
    signals.push({ type: 'volume_velocity', strength: 'moderate', description: `Vol/Liq ${(token.volume24h / token.liquidity).toFixed(1)}x`, points: 10 })
  }
  ...
  if (buyPressure < 0) {
    ...
    score += 16
    signals.push({ type: 'buy_pressure', strength: 'strong', description: `Buy pressure ${buyPressure}%`, points: 16 })
  } else if (buyPressure >= 55) {
    score += 6
    signals.push({ type: 'buy_pressure', strength: 'weak', description: `Buy pressure ${buyPressure}%`, points: 6 })
  } else {
  ...
  if (token.txns24h) {
    ...
    if (txnTotal >= 500) {
      score += 8
      signals.push({ type: 'organic_activity', strength: 'moderate', description: `${txnTotal} txs in 24h`, points: 8 })
    }
  }
  ...
  if (socialCount >= 2) {
    score += 10
    signals.push({ type: 'socials', strength: 'strong', description: `${socialCount} social links`, points: 10 })
  } else if (config.requireSocials && socialCount === 0) {
    warnings.push('No social links found')
  }

  // --- Narrative / symbol heuristics ---
  const symbol = token.symbol.toLowerCase()
  const narrativeMatches = symbol.includes('pepe') || symbol.includes('inu') || symbol.includes('cat') || symbol.includes('moon')
  if (narrativeMatches) {
    score += 4
    signals.push({ type: 'narrative', strength: 'weak', description: 'Matches runner-like narrative pattern', points: 4 })
  }

  // --- Low-liquidity penalty (manipulation risk) ---
  ...
  // --- Perfect-storm bonus: when 3+ extreme signals stack, this is a real runner ---
  const extremeCount = signals.filter(s => s.strength === 'extreme').length
  if (extremeCount >= 3) {
    score += 10
    signals.push({ type: 'perfect_storm', strength: 'extreme', description: `3+ extreme signals stacked (+10)`, points: 10 })
  }
